...
def eval_one(annotation_filename, run_results_dirpath, cocoGt, config, annType, pool=None):
    ...
    if not os.path.exists(metrics_filepath):
        ...
        if len(cocoDt.loadAnns(cocoDt.getAnnIds(imgIds=cocoDt.getImgIds()[0]))) > 0:
            if isinstance(cocoDt.loadAnns(cocoDt.getAnnIds(imgIds=cocoDt.getImgIds()[0]))[0]["segmentation"], list):
                ...
                metrics["avg_max_angle_diffs"] = sum(list(max_angle_diffs)) / (len(list(max_angle_diffs)))
...
def compute_contour_metrics(gts_dts):
    ...
    _max_angle_diffs = polygon_utils.compute_polygon_contour_measures(fixed_gt_polygons, fixed_dt_polygons, sampling_spacing=2.0, min_precision=0.5, max_stretch=2)
    max_angle_diffs.extend(_max_angle_diffs)
