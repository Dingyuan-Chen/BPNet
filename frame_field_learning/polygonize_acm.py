...
class PolygonAlignLoss:
    def __init__(self, indicator, edge_batch, level, c0c2, data_coef, length_coef, crossfield_coef, dist=None, dist_coef=None):
    ...
    def __call__(self, tensorpoly):
        ...
        pos_edge_value = bilinear_interpolate(self.edge_batch[:, None, ...], tensorpoly.pos, batch=tensorpoly.batch)
        level_edge_loss = torch.sum(torch.pow(pos_edge_value - 1., 2))
...
class TensorPolyOptimizer:
    def __init__(self, config, tensorpoly, indicator, edge_batch, c0c2, data_coef, length_coef, crossfield_coef, dist=None, dist_coef=None):
        ...
        self.criterion = PolygonAlignLoss(indicator, edge_batch, config["data_level"], c0c2, data_coef, length_coef,
                                          crossfield_coef, dist=dist, dist_coef=dist_coef)
...
def polygonize(seg_batch, crossfield_batch, config, pool=None, pre_computed=None):
    ...
    edge_batch = seg_batch[:, 1, :, :]
    np_edge_batch = edge_batch.cpu().numpy()
    ...
    tensorpoly_optimizer = TensorPolyOptimizer(config, tensorpoly, indicator_batch, edge_batch, crossfield_batch,
                                               config["data_coef"],
