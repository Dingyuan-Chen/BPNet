...
def get_backbone(backbone_params):
    ...
    elif backbone_params["name"] == "pspnet":
        from torchvision.models.segmentation._utils import _SimpleSegmentationModel
        from frame_field_learning.pspnet import PSPNet

        backbone = PSPNet(backend=backbone_params["backend"])
        backbone = _SimpleSegmentationModel(backbone, classifier=torch.nn.Identity())
    elif backbone_params["name"] == "hrnet":
        from torchvision.models.segmentation._utils import _SimpleSegmentationModel
        from frame_field_learning.hrnet import HRNetHead
